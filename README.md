...
Docker Compose로 App + DB 2-Tier 구성을 띄우며 **시작 순서 제어**와 **빌드 통합**을 배우는 실습입니다. 

## 사전 요구사항

- Docker Engine + **Docker Compose v2** (플러그인)
  - 확인: `docker compose version` → `Docker Compose version v2.x` 이상
  - 이 실습의 모든 명령은 `docker-compose`(하이픈, v1)가 아니라 **`docker compose`**(공백, v2)를 사용합니다. v1은 EOL(End of Life)되었음!
- 3000번 포트가 비어 있을 것
